...
class ResilienceTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_default_provider_contract_is_three_independent_bounded_providers(self):
        previous = {key: os.environ.get(key) for key in (
            "AI_PROVIDERS_JSON",
        ...
        try:
            ...
            self.assertEqual([p.timeout_ms for p in cascade.providers], [9000, 9000, 9000])
            ...
            self.assertEqual([p.model for p in cascade.providers], ["kilo-auto/free", "auto", "openai"])
